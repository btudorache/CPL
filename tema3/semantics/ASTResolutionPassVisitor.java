...
public class ASTResolutionPassVisitor implements ASTVisitor<ResolutionResult> {
    ...
    public ResolutionResult visit(FunctionDefinition functionDefinition) {
        if (!functionDefinition.hasSemanticError()) {
            ...
            for (var param : functionDefinition.formalParams) {
                param.accept(this);
                param.symbol.idType = IdSymbolType.FormalIdSymbol;
                param.symbol.offset = 12 + 4 * offsetIndex;
                offsetIndex++;
            }
...
//            if (leftType.getName().eq)
    ...
    public ResolutionResult visit(Let let) {
        ...
        for (var param : let.params) {
            param.symbol.idType = IdSymbolType.LocalIdSymbol;
            param.symbol.offset = -(offset * 4);
            offset++;
            param.accept(this);
        }
    ...
    public ResolutionResult visit(CaseBranch caseBranch) {
        if (!caseBranch.hasSemanticError()) {
            ...
            idSymbol.idType = IdSymbolType.LocalIdSymbol;
            idSymbol.offset = -4;
